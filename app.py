...
# df_matches = df_matches.chat("drop the columns 'umpire1', 'umpire2', and 'umpire3'")
# ipl_winner = df_matches.chat("group the matches by season, and then count the number of win for each team in each season ")
# select_year = df_matches.chat("Create a list of unique seasons from the 'Season' column in ascending order. Include an 'Overall' option at the beginning of the list")
...
if user_menu == 'IPL Matches':
    ...
    if selected_year == 'Overall':
        data = {"Category": ["Matches", "Participated Teams", "Most Championship Wins", "Participating Country", "Participated Players", "Business value"], "Value": [786, 15, "Mumbai Indians", 11, 566, "$5.7 billion"]}
        ...
        data = {'N': [1, 2, 3, 4, 5, 6, 7],
                ...
                'Estimated sponsorship fee': ['US$5.0 million', 'US$9.9 million', 'US$12.5 million', 'US$55.1 million', 'US$27.8 millon',
                                              'US$55.1 million', 'US$37.6–44.5 million']}
    ...
    if selected_year != 'Overall':
        ...
        with col1:
            st.header('Winner')
            st.title(team['winner'].tolist()[-1])
        with col2:
            st.header('Man Of The Match')
            st.title(team['player_of_match'].tolist()[-1])
